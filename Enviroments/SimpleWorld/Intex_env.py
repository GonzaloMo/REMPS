...
def create_env(n_plan_states = 10,  Reward_type='Eval', eval_p_list=['Train_env'], prob_type = 'Train_env'):
    env = Env_Wrap(n_plan_states=n_plan_states, Reward_type=Reward_type, problem_type=prob_type)
    eval_envs  = {}
    for p_t in eval_p_list:
        eval_envs[p_t] = Env_Wrap(n_plan_states=n_plan_states, problem_type=p_t,env_type='Eval_'+prob_type)
    return env, eval_envs

class Env_Wrap(gym.Env):
    metadata = {'render.modes': ['human']}               
        
    def __init__(self, n_plan_states = 10, problem_type='Train_env', verbose=False, debug=False, render_type = 'PYGAME',  Reward_type='Eval', env_type='Train'):
        ...
        ''' initialize world '''
        self.gridworld = Gridworld(problem_type=problem_type, debug=debug, verbose=verbose, render_type =render_type)
        self.num_steps = 0
