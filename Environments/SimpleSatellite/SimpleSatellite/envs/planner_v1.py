...
class Simple_satellite(gym.Env):
    def __init__(self,
        ...
        #\{
        # A plan state should have the minimum information needed for the agent to know the plan state it should achieve. 
        # the sub set of the state of the world send as plan state is called the latent state
        # the latent space is any part of the state which is modified by the agent and not the environment
        # In case of theis environment the latent space is:
        #   - Delta Images
        #   - Delta Analysis
        #   - Delta Goals
        # \}
        latent_space = {'Images':spaces.Box(low=0, high=max_inf, shape=(n_targets,), dtype=np.int32),# n images per target taken
                        'Analysis':spaces.Box(low=0, high=max_inf, shape=(n_targets,), dtype=np.int32), # n images per target analyzed
                        'Goals':spaces.Box(low=0, high=max_inf, shape=(n_targets,), dtype=np.int32),}
        plan_space = {}
        for i in range(number_of_states):
            plan_space[f"Plan_{i}"] = spaces.Dict(latent_space)
        obs_space = {**state_space, **plan_space}
    ...
    def get_plan_state(self) -> Dict[str, Any]:
        ...
        Output:
            observation: Dict[str, Any]
        """
        states = self.planner.get_planner_states(self.number_of_states)
        observation = {}
        for i in range(1, len(states)): 
            observation[f"Plan_{i}"] = {"Goals":np.array(states[i]["Goals"], dtype=np.int32) - np.array(states[i-1]["Goals"], dtype=np.int32),
                                        "Analysis":np.array(states[i]["Analysis"], dtype=np.int32) - np.array(states[i-1]["Analysis"], dtype=np.int32),
                                        "Images":np.array(states[i]["Images"], dtype=np.int32) - np.array(states[i-1]["Images"], dtype=np.int32),}
        return observation

    def get_obs(self) -> Dict[str, Any]:
        state = self.SatSim.get_current_state()
        plan_state = self.get_plan_state()
        observation = {**state, **plan_state}
        return observation
