...
class Gridworld:
    ...
    def render_path(self, path, colorname="b", width=2):     
        if type(colorname) == str:
            c = self.ColorStr2RGB(colorname)   
            cl = False
        else:
            cl = True
        block_size = Gridworld.block_size*Gridworld.Scale
        for i in range(0, len(path)):
            if cl:
                c = self.ColorStr2RGB(colorname[i])
            x,y = path[i]
    ...
    def ColorStr2RGB(self, color):
        if color == "k":
            return Gridworld.black
        elif color == "lb":
            return Gridworld.light_blue
        elif color == "r":
            return Gridworld.red
        elif color == "lr":
            return Gridworld.light_red
        elif color == "g":
            return Gridworld.green
        elif color == "b":
            return Gridworld.blue
        elif color == "gr":
            return Gridworld.grey
        elif color == "w":
            return Gridworld.white
        else:
            raise ValueError(f"Color {color} not defined")
