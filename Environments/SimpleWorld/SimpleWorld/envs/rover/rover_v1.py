...
class Gridworld_Rover_env(gym.Env):
    ...
    def reset(self):
        ...
        self.Map = self.sim.Create_empty_map()
        self.GoalMap = np.zeros(self.Map.shape)
        self.Map = self.sim.generate_obstacles(self.Map, n_obstacle)
        self.Base = list(np.random.randint(0, high=self.grid_size, size=2))
        self.generateGoals()
        self.info = {}
